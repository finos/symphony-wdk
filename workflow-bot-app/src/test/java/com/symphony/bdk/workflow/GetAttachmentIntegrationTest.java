...
class GetAttachmentIntegrationTest extends IntegrationTest {
  ...
  void getAttachment() throws Exception {
    ...
    assertFileExistsAndDeleteIt(expectedFilePath);
  }
  ...
  private void assertFileExistsAndDeleteIt(String filePath) {
    try {
      File file = new File(filePath);
      if (file.exists()) {
        boolean isFileDeleted = file.delete();

        // added this to get file.delete return value not ignored, otherwise a SpotBugsExecutor warning will be raised
        if (isFileDeleted) {
          log.info("File {} generated by tests has been deleted", filePath);
        } else {
          log.info("Failed to delete file {} generated by tests", filePath);
        }
      } else {
        fail("No file found in the expected location");
      }
    } catch (Exception exception) {
      fail("An error occurred while checking the file, this should not happen");
    }
  }
